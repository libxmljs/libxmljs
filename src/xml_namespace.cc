...
NAN_METHOD(XmlNamespace::New)
{
  ...
  XmlNode *node = Nan::ObjectWrap::Unwrap<XmlNode>(info[0]->ToObject(v8::Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked());
  ...
  if (info[1]->IsString())
  {
    prefix = new v8::String::Utf8Value(v8::Isolate::GetCurrent(), info[1]);
  }

  href = new v8::String::Utf8Value(v8::Isolate::GetCurrent(), info[2]);
...
void XmlNamespace::Initialize(v8::Local<v8::Object> target)
{
  Nan::HandleScope scope;
