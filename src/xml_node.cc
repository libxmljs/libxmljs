    ...
    NAN_METHOD(XmlNode::ToString) {
            ...
            if (info.Length() > 0) {
                if (info[0]->IsBoolean()) {
                    ...
                    v8::Local <v8::Value> c14n = obj->Get(Nan::New<v8::String>("c14n").ToLocalChecked());
                    // canonicalization
                    if (c14n->Equals(Nan::New<v8::String>("excl").ToLocalChecked())) {
                        c14nOpt = 1;
                    } else if (c14n->Equals(Nan::New<v8::String>("incl").ToLocalChecked())) {
                        c14nOpt = 0;
                    }
            ...
            if ((xml_obj != skip_xml_obj) && (xml_obj->_private != NULL)) {
                ...
                if ((wrapped_node == NULL) &&
                    ...
                    if ((wrapped_node == NULL) && (xml_obj->properties != NULL)) {
                        wrapped_node =
                                reinterpret_cast<xmlNode *>(get_wrapped_attr_in_list(xml_obj->properties,
                                                                                     skip_xml_obj));
                    }
 ...
 */
    ...
    void XmlNode::copyParentNamespaces(xmlNode *c_from_node, xmlNode *c_to_node) {
        xmlNode *c_parent;
        xmlNs *c_ns;
        xmlNs *c_new_ns;
    ...
    void XmlNode::destroyFakeDoc(xmlDoc *c_doc) {
        xmlDoc *c_base_doc = doc;
        if (c_doc == c_base_doc) return;
        xmlNode *c_child;
        xmlNode *c_parent;
        xmlNode *c_root;
